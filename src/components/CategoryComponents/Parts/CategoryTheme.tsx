...
const setLocalStorage = (theme:Theme, isOpen:boolean) => {
  switch (theme) {
    case Theme.genre:
...
const initSetting = (theme:Theme) => {
  switch (theme) {
    case Theme.genre:
      return !!localStorage.getItem('genre') ? !!localStorage.getItem('genre'): localStorage.getItem('genre') === null;
    case Theme.member:
      return !!localStorage.getItem('member') ? !!localStorage.getItem('member'): localStorage.getItem('member') === null;
    default:
      return true;
  }
}
...
export const CategoryTheme: React.FC<Props> = ({themeOption}) => {
  ...
  return (
    <ul className='py-2 mr-2 border-b border-gray-600 border-dashed'>
      ...
      {isOpen ? (
        allCategory.filter(value => value.theme === themeOption.value).map(categoryInfo => (
          <CategoryItem key={categoryInfo.id} categoryInfo={categoryInfo} />
        ))
      ):(
        ''
      )}
