...
const TagForm: React.FC<Props> = ({ inputTagStr, setInputTagStr, inputTagList, setInputTagList }) => {
    ...
    >
      <div className="mb-1">
        {inputTagList.map((value, index) => (
          <span
            key={index}
            className="inline-block pl-2 py-0.5 border border-gray-400 rounded-2xl bg-blue-100 font-bold mr-1 mt-1 shadow-md"
          >
            {value}
            <span
              className="bg-white m-1 px-1 bg-gray-400 rounded-full font-bold text-white"
              onClick={() => {
      ...
      <label className="inline-block w-12" htmlFor="inputTag">
        タグ：
      </label>
      <input
