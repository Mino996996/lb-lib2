import {createInitTagList, isCategoryTag, pickRelationalTabs, testABC, urlPattern} from "./cardFunctions";
...
describe('cardFunctions.ts', ()=> {
  ...
  it('選択されたカテゴリに含まれるタグを取り出す', () => {
    const result_1 = ['AWS', 'test', 'test2', '寿司安価スレ'];
    const result_2 = ['test', 'test2', '寿司安価スレ'];
    expect(pickRelationalTabs(urlInfoList, 'test')).toEqual(result_1);
    expect(pickRelationalTabs(urlInfoList, '寿司安価スレ')).toEqual(result_2);
  });
  ...
  it('登録済みタグと選択カテゴリの状態に合わせた初期のタグリストを返す', ()=>{
    ...
    const result_1 = ['AWS', 'EC2', '設定方法', 'データベース'];
    expect(createInitTagList(tagList, 'データベース')).toEqual(result_1);
