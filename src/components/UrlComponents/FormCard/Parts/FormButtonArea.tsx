...
const FormButtonArea: React.FC<Props> = (prop) => {
  ...
  return (
    <div className='ml-2 text-center'>
      {prop.mode === 'update' && (
        <button
          className={'px-4 py-1 mr-16 bg-black font-bold text-white rounded '}
      ...
      <button
        className={'px-4 py-1 bg-purple-600 font-bold text-white rounded ' + (prop.inputTitle ? '' : 'bg-gray-300 text-gray-600')}
        onClick={prop.post} disabled={!prop.inputTitle}
      >
