...
const blankUrlInfo: UrlInfo = {id: '', title:'', url: '', pageTitle:'', pageImage:'', pageDescription:'', tagList:[], memo:'', addTime:0, fileUrl:'', fileName:'', fileId:''};
...
const UrlArea: React.VFC<Props> = () => {
  ...
  useEffect(() => {
    ...
    if (!!keywords.length && !!selectedCategory) { // 両方データあり
    ...
    } else if (!!keywords.length && !selectedCategory) { // keywordsのみ
      for(const keyword of keywords) {
        filteredList = filteredList.filter(value => value.tagList.includes(keyword));
      }
    } else if (!keywords.length && !!selectedCategory) { // selectedCategoryのみ
